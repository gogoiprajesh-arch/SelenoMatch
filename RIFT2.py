...
from phasecong_gpu import phasecong3_gpu

def grid_nms(keypoints, image_shape, grid_rows=4, grid_cols=4, max_per_cell=30):
    """
    Forces spatial distribution by keeping only the strongest keypoints within each local grid sector.
    """
    if not keypoints:
        return []
    ...
    # Create empty bins for each grid cell
    grid = [[[] for _ in range(grid_cols)] for _ in range(grid_rows)]

    # Assign keypoints to their respective geographic bins
    for kp in keypoints:
    ...
    # Sort keypoints within each bin by strength (response) and keep the top N
    for r in range(grid_rows):
        for c in range(grid_cols):
            cell_kps = grid[r][c]
            if cell_kps:
                # Sort descending by FAST response score
                cell_kps.sort(key=lambda x: x.response, reverse=True)
                distributed_keypoints.extend(cell_kps[:max_per_cell])
...
class RIFT2:
    ...
    def feature_detection(self, im):
        ...
        # 2. Call the GPU vectorized function
        m, eo = phasecong3_gpu(im, nscale=config['nscale'], norient=config['norient'], 
        ...
        # PyTorch returns (norient, nscale, H, W). 
        # The RIFT descriptor loop expects (nscale, norient, H, W).
        eo = np.transpose(eo, (1, 0, 2, 3))

        fast = cv2.FastFeatureDetector_create(threshold=13, nonmaxSuppression=True)
        keypoints = fast.detect(m_image, None)

        keypoints = grid_nms(keypoints, m_image.shape, grid_rows=4, grid_cols=4, max_per_cell=30)
        
        if len(keypoints) > 0:
            kpts = np.array([kp.pt for kp in keypoints]).T
        else:
            kpts = np.zeros((2, 0), dtype=np.float32)
