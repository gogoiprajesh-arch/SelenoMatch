...
Usage:
    python run_pipeline.py \
        ...
        --loftr-repo ./LoFTR --loftr-weights ./LoFTR/weights/outdoor_ds.ckpt \
        --out-dir output
"""
import argparse
import json
...
from geosampler import sample_overlap, sample_unprojected_xml
...
def run_pipeline(ref_path, src_path, loftr_repo_dir, loftr_weights, out_dir="output",
    ...
    target_res = overlap["target_res"]
    
    

    # Safely load Reference
    if str(ref_path).endswith(".xml"):
        raw_ref = sample_unprojected_xml(ref_path, meta_ref['corners'], out_shape, bounds, target_res, is_iirs=False)
    else:
        raw_ref = sample_overlap(ref_path, overlap["bbox1"], out_shape)

    # Safely load Source
    if str(src_path).endswith(".xml"):
        print(f"       [XML Mode] Orthorectifying source to Universal Grid (IIRS={is_iirs})...")
        raw_src = sample_unprojected_xml(src_path, meta_src['corners'], out_shape, bounds, target_res, is_iirs)
    else:
        raw_src = sample_overlap(src_path, overlap["bbox2"], out_shape)
    ...
    # RIFT-2 gets the macro-patch whole; LoFTR quarters it into 512x512
    # micro-patches internally (see features.LoFTRRunner) and hands back
    # results already re-stitched into the macro-patch frame, so fusion
    # below never needs to know a quadrant split happened.
    ...
    for i, tile in enumerate(tqdm(tiles, desc="tiles")):
        ...
        # --- THE SMART CAPPER ---
        # Keep ALL elite RIFT-2 points, but limit LoFTR to its top 150 to kill noise
        ...
        # Force CPU RAM cleanup (ONLY EVERY 15 TILES)
        if i % 15 == 0:
            gc.collect()
            
            # Force GPU VRAM cleanup to prevent WSL2 fragmentation
...
if __name__ == "__main__":
    ...
    run_pipeline(
        ...
        is_iirs=False                   # Dynamic thermal trigger
    )